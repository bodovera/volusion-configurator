(function () {
  ...
  function getPriceSelect() {
    return document.querySelector('select[class*="doogma-price_"]');
  }
  ...
  function hidePriceSelect() {
    const priceSelect = getPriceSelect();
    if (!priceSelect) return;

    priceSelect.style.display = "none";
    priceSelect.setAttribute("aria-hidden", "true");

    let row = priceSelect.parentElement;
    while (row && row !== document.body) {
      const text = (row.textContent || "").trim().toUpperCase();
      if (text.indexOf("VALUES") >= 0 || text.indexOf("PRICE") >= 0) {
  ...
  function init() {
    ...
    setTimeout(hidePriceSelect, 500);
    setTimeout(hidePriceSelect, 1000);
    setTimeout(hidePriceSelect, 1500);
  }
