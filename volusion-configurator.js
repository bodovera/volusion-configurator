(function () {
  ...
  function refreshAll() {
    ...
    try {
      ...
      // 🔥 ONLY ADDITION: sync boxes AFTER rules run
      syncOptionBoxes();
  ...
  // =========================
  // OPTION BOXES (SAFE LAYER)
  // =========================

  function buildOptionBoxes() {
    document.querySelectorAll("strong[role='heading']").forEach(function (heading) {
      const group = heading.parentElement;
      if (!group || group.dataset.boxReady === "1") return;

      const inputs = group.querySelectorAll("input[data-doogma-value]");
      if (!inputs.length) return;

      const container = document.createElement("div");
      container.className = "bod-option-boxes";
      ...
      inputs.forEach(function (input) {
        ...
        box.innerHTML = `<div class="bod-option-box-text">${text}</div>`;

        box.onclick = function () {
          if (input.type === "radio") {
            document.querySelectorAll(`input[name="${input.name}"]`).forEach(r => r.checked = false);
            input.checked = true;
          } else {
      ...
      group.dataset.boxReady = "1";
    });
  }

  function syncOptionBoxes() {
    document.querySelectorAll(".bod-option-box").forEach(function (box) {
      const input = document.getElementById(box.dataset.inputId);
      if (!input) return;

      const wrapper = getFieldWrapper(input);
      box.style.display = wrapper && wrapper.style.display !== "none" ? "" : "none";
  ...
  async function init() {
    try {
      ...
      buildOptionBoxes(); // build once
      refreshAll();       // rules + sync
